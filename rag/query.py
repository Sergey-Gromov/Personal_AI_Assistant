...
RAG_SEARCH_TIMEOUT_SECONDS = 20
RAG_LLM_TIMEOUT_SECONDS = 45
RAG_FALLBACK_TIMEOUT_SECONDS = 30
...
async def query_knowledge_base(
    ...
    try:
        ...
        logger.info(f"RAG: searching knowledge base for query: {query[:80]}")
        ...
        logger.warning("RAG timeout, using fallback response")
        return await _fallback_response(query, conversation_history)
...
Используй этот контекст для ответа на вопрос пользователя."""
    ...
    response = await asyncio.wait_for(
        openai_client.generate_text_response(messages),
...
что это не основано на специфической базе знаний."""
    ...
    try:
        response = await asyncio.wait_for(
            openai_client.generate_text_response(messages),
            timeout=RAG_FALLBACK_TIMEOUT_SECONDS
        )
        return f"⚠️ База знаний не содержит информации по этому вопросу.\n\n{response}"
    except asyncio.TimeoutError:
        logger.error("Fallback response timed out")
        return (
            "⚠️ База знаний сейчас отвечает слишком долго.\n\n"
            "Попробуйте еще раз через минуту или переключитесь в режим /mode text."
        )
