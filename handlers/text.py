...
async def handle_text_message(message: types.Message):
    ...
    try:
        if mode == BotMode.RAG:
            rag_status_message = await bot.send_message(
                message.chat.id,
                "📚 Ищу информацию в базе знаний..."
            )

        # Route request
        response = await route_text_request(user_id, text, mode=mode)
        
        # Check if response contains an image
        if response.get('has_image') and response.get('image_path'):
            if rag_status_message:
                try:
                    await bot.delete_message(message.chat.id, rag_status_message.message_id)
                except Exception:
                    pass
