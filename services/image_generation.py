...
async def detect_image_generation_intent(
    ...
    strong_keywords = [
        ...
        'generate image', 'draw', 'create image', 'create an image', 'create picture',
        'make image', 'show me what',
        'сгенерируй картинку', 'создай изображение', 'сделай картинку'
    ]
    context_keywords = [
        'покажи', 'показать', 'как выглядит', 'show', 'visualize', 'what it looks like'
    ]
...
"""
    ...
    try:
        ...
        response = await openai_client.generate_text_response(messages, temperature=0.3, request_id=req)
        ...
        logger.info(
            "[req:%s] Image generation detection: %s (confidence: %s)",
            req,
            result.get('needs_generation'),
        ...
        req = request_id or "n/a"
        logger.error("[req:%s] Error detecting image generation intent: %s", req, e)
        
        # Fallback: use keyword detection
        if has_strong_keyword or (conversation_history and any(keyword in text_lower for keyword in context_keywords)):
            return {
...
async def generate_image(
    ...
    try:
        req = request_id or "n/a"
        logger.info("[req:%s] Generating image with DALL-E: %s...", req, prompt[:100])
        ...
        logger.info("[req:%s] Image generated successfully. Revised prompt: %s...", req, revised_prompt[:100])
        ...
        req = request_id or "n/a"
        logger.error("[req:%s] Error generating image: %s", req, e)
        raise
